...
import { UploadCloud, RefreshCw, Grid, List, Search, Filter, SlidersHorizontal, Image as ImageIcon } from 'lucide-react';
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { title: 'Total Photos', value: photos.length, icon: <ImageIcon className="h-5 w-5 text-primary-500" />, color: 'bg-primary-50 dark:bg-primary-900/20' },
          { title: 'Shared Photos', value: 3, icon: <Filter className="h-5 w-5 text-accent-500" />, color: 'bg-accent-50 dark:bg-accent-900/20' },
          { title: 'Storage Used', value: '128 MB', icon: <SlidersHorizontal className="h-5 w-5 text-purple-500" />, color: 'bg-purple-50 dark:bg-purple-900/20' },
          { title: 'Recent Uploads', value: '2 today', icon: <UploadCloud className="h-5 w-5 text-green-500" />, color: 'bg-green-50 dark:bg-green-900/20' }
        ].map((stat, index) => (
          <div key={index} className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
      ...
      {/* Photos section */}
      <div id="photos-section" className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700">
          ...
          <div className="text-sm text-gray-500 dark:text-gray-400">
            {searchQuery ? (
              ...
              filteredPhotos.length > 0 && (
                <>Showing {Math.min((currentPage - 1) * photosPerPage + 1, filteredPhotos.length)}-{Math.min(currentPage * photosPerPage, filteredPhotos.length)} of {filteredPhotos.length}</>
              )
            )}
          </div>
        </div>
